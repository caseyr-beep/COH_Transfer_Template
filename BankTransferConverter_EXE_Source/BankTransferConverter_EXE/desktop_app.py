...
def read_mapping(path):
    """Read either horizontal row1-bank/row2-account/row3-code or 2-column mapping."""
    if not path:
        return {}
    ...
    if df.shape[1] >= 2:
        for _, row in df.iterrows():
            ...
            acct = last4(a)
            code = str(b).strip()
            if acct.isdigit() and len(acct) == 4 and code and "bank code" not in code.lower():
                mapping[acct] = code

    # Horizontal format: row 1 bank, row 2 account number, row 3 code
    if df.shape[0] >= 3:
        for col in range(df.shape[1]):
            acct = df.iloc[1, col]
            code = df.iloc[2, col]
            if pd.isna(acct) or pd.isna(code):
                continue
            acct4 = last4(acct)
            code = str(code).strip()
            if acct4.isdigit() and len(acct4) == 4 and code:
                mapping[acct4] = code
...
def convert_file(raw_csv, mapping_path, output_path, extra_scrub_text=""):
...
class ConverterApp(tk.Tk):
    ...
    def _build_ui(self):
        ...
        ttk.Label(frm, text="2. Bank-code mapping workbook").grid(row=2, column=0, sticky="w", **pad)
        ttk.Entry(frm, textvariable=self.mapping_path, width=70).grid(row=3, column=0, sticky="ew", padx=12)
        ttk.Button(frm, text="Browse...", command=self.pick_mapping).grid(row=3, column=1, padx=6)

        ttk.Label(frm, text="3. Output workbook").grid(row=4, column=0, sticky="w", **pad)
        ttk.Entry(frm, textvariable=self.output_path, width=70).grid(row=5, column=0, sticky="ew", padx=12)
        ttk.Button(frm, text="Save As...", command=self.pick_output).grid(row=5, column=1, padx=6)

        ttk.Label(frm, text="Additional scrub pairs (optional, one pair per line: 1111,2222)").grid(row=6, column=0, sticky="w", **pad)
        self.extra_scrubs = tk.Text(frm, height=5, width=60)
        self.extra_scrubs.grid(row=7, column=0, columnspan=2, sticky="nsew", padx=12)
        ...
        ttk.Label(frm, text=rules, wraplength=690).grid(row=8, column=0, columnspan=2, sticky="w", padx=12, pady=12)

        self.convert_btn = ttk.Button(frm, text="Convert", command=self.run_conversion)
        self.convert_btn.grid(row=9, column=0, sticky="w", padx=12, pady=10)
        ttk.Label(frm, textvariable=self.status).grid(row=9, column=0, sticky="e", padx=12)
    ...
    def run_conversion(self):
        ...
        if not mapping or not os.path.exists(mapping):
            messagebox.showerror("Missing file", "Please select the bank-code mapping workbook.")
            return
        if not output:
        ...
        def worker():
            try:
                raw_count, output_rows, scrubbed_count, missing_codes = convert_file(raw, mapping, output, extra)
                msg = (
